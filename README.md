...
```bash
git clone <juicebox-repo>claude-agents.git
```
...
```bash
ln -sf "$(pwd)/claude-agents" ~/.claude/claude-agents
```

**Windows (PowerShell):**

```powershell
cmd /c mklink /D "$env:USERPROFILE\.claude\claude-agents" "$(Get-Location)\claude-agents"
```
